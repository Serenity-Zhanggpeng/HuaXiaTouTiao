...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    @PostConstruct              //表示在 Spring 容器初始化时执行该方法。
    private void init() {
        SECKILL_ORDER_EXECUTOR.submit(() -> {
            ...
            while (true) {
                try {
                    //从消息队列中获取订单信息
                    //获取消费队列中的订单消息 xreadgroup group g1 c1 count 1 bloac 2000 streams stream.orders >
                    List<MapRecord<String, Object, Object>> list = stringRedisTemplate.opsForStream().read(
                            Consumer.from("g1", "c1")
                            , StreamReadOptions.empty().count(1).block(Duration.ofSeconds(2))
                            , StreamOffset.create(queueName, ReadOffset.lastConsumed())  //从末尾读取
                    );
                    //判断消息时候获取成功
                    ...
                    e.printStackTrace();
                    handlePendingList();  //消息异常处理
                }
            }
        });
    }

    //获取pending-list里的异常信息 xreadgroup group g1 c1 count 1 bloac 2000 streams stream.orders 0
